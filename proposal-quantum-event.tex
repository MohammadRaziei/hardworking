			...
			\begin{tikzpicture}
				...
				align=right
					...
					\begin{tabular}{r l}
						...
						\lr{Quantum Tech} — همایش تخصصی مخابرات کوانتومی و رمزنگاری پساکوانتومی $1405$ \\[6pt]
...
\speakercard
	...
	\begin{tikzpicture}[
		...
		\node[slot] at (8,\ypos) {\textbf{مراسم افتتاحیه} - معرفی مرکز تحقیق و نوآوری همراه اول};
		...
		\node[slot] at (8,\ypos) {\textbf{سخنرانی ششم} - کیوبیت‌های ابررسانا و تصحیح خطای کوانتومی \textbar\ دکتر محمدحسین انصاری};
	...
	\begin{center}
		\begin{tikzpicture}[
			...
			\node[slot] at (8,\ypos) {\textbf{مراسم افتتاحیه} - معرفی مرکز تحقیق و نوآوری};
	...
	\begin{center}
		\begin{tikzpicture}[
			...
			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$9:20$ - $10:05$};
			\node[slot] at (8,\ypos) {\textbf{سخنرانی اول} - گذار به رمزنگاری پساکوانتومی و استانداردهای NIST \textbar\ دکتر ترانه اقلیدس};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$10:05$ - $10:50$};
			\node[slot] at (8,\ypos) {\textbf{سخنرانی دوم} - زیرساخت‌های مخابرات نوری و QKD \textbar\ دکتر جواد صالحی};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$10:50$ - $11:10$};
			\node[break] at (8,\ypos) {پذیرایی - استراحت};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$11:10$ - $11:55$};
			\node[slot] at (8,\ypos) {\textbf{سخنرانی سوم} - مبانی نظری اطلاعات کوانتومی و شبکه‌های کوانتومی \textbar\ دکتر علی رضاخانی};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$11:55$ - $12:40$};
			\node[slot] at (8,\ypos) {\textbf{سخنرانی چهارم} - \lr{QKD} و معماری شبکه‌های کوانتومی در سطح جهانی \textbar\ دکتر محسن رضوی (خارجی)};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$12:40$ - $13:40$};
			\node[break] at (8,\ypos) {ناهار و نماز};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$13:40$ - $14:25$};
			\node[slot] at (8,\ypos) {\textbf{سخنرانی پنجم} - از رمزنگاری کلاسیک تا رمزنگاری پساکوانتومی \textbar\ دکتر ریحانه صفوی‌نائینی (خارجی)};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$14:25$ - $15:10$};
			\node[slot] at (8,\ypos) {\textbf{سخنرانی ششم} - پیاده‌سازی عملی پروتکل‌های مخابرات کوانتومی \textbar\ محمد رضایی};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$15:10$ - $15:25$};
			\node[break] at (8,\ypos) {پذیرایی - استراحت};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$15:25$ - $16:10$};
			\node[slot,draw=mci-gold,fill=mci-gold!8] at (8,\ypos)
			{\textbf{\color{mci-gold}رونمایی از کتاب} - ارتباطات کوانتومی، شبکه‌های کوانتومی و حسگری کوانتومی};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$16:10$ - $17:00$};
			\node[slot,fill=mci-blue!8,draw=mci-blue] at (8,\ypos)
			{\textbf{\color{mci-blue}جمع‌بندی روز اول} - پرسش و پاسخ};
	...
	\begin{center}
		\begin{tikzpicture}[
			...
			\node[timebox] at (0,\ypos) {$9:00$ - $9:15$};
			\node[break] at (8,\ypos) {خوش‌آمدگویی و مرور روز اول};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$9:15$ - $10:00$};
			\node[slot] at (8,\ypos) {\textbf{سخنرانی اول} - فناوری‌های نوین حسگری کوانتومی و کاربردهای صنعتی \textbar\ امیر یوسفی};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$10:00$ - $10:45$};
			\node[slot] at (8,\ypos) {\textbf{سخنرانی دوم} - اپتیک کوانتومی و بوسون سمپلینگ \textbar\ دکتر صالح رحیمی‌کشری (خارجی)};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$10:45$ - $11:00$};
			\node[break] at (8,\ypos) {پذیرایی - استراحت};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$11:00$ - $11:45$};
			\node[slot] at (8,\ypos) {\textbf{سخنرانی سوم} - مبانی نظری محاسبات و اطلاعات کوانتومی \textbar\ دکتر وحید کریمی‌پور};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$11:45$ - $12:30$};
			\node[slot] at (8,\ypos) {\textbf{سخنرانی چهارم} - کیوبیت‌های ابررسانا و تصحیح خطای کوانتومی \textbar\ دکتر محمدحسین انصاری (خارجی)};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$12:30$ - $13:30$};
			\node[break] at (8,\ypos) {ناهار و نماز};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$13:30$ - $14:15$};
			\node[slot] at (8,\ypos) {\textbf{سخنرانی پنجم} - محاسبات کوانتومی ابری و \lr{Blind Quantum Computing} \textbar\ دکتر الهام کاشفی (خارجی)};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$14:15$ - $15:00$};
			\node[slot] at (8,\ypos) {\textbf{سخنرانی ششم} - مخابرات کوانتومی، حسگری و شبکه‌های کوانتومی \textbar\ دکتر ایوان جرجودیک (خارجی)};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$15:00$ - $15:15$};
			\node[break] at (8,\ypos) {پذیرایی - استراحت};

			\pgfmathsetmacro\ypos{\ypos+\ystep}
			\node[timebox] at (0,\ypos) {$15:15$ - $16:00$};
			\node[slot,fill=mci-blue!8,draw=mci-blue] at (8,\ypos)
			{\textbf{\color{mci-blue}اختتامیه} - جمع‌بندی دو روز، پرسش و پاسخ، عکس یادگاری};
